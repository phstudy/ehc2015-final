...
# 指定 max_cat 為無序的類別變數
train$max_cat <- factor(train$max_cat, levels = c("0","1","A","B","C","D","E","F","G","H","I","J","K","L","O","V", "_"))
...
names(test_data) <- c("eruid","viewcount","uniq_viewcount","cat_0","cat_1","cat_A","cat_B","cat_C","cat_D","cat_E","cat_F","cat_G","cat_H","cat_I","cat_J","cat_K","cat_L","cat_O","cat_V","max_cat","buyCount","buy")

# 轉成 data frame 並指定 eruid 為 row id
test_data <- data.frame(test_data, row.names = "eruid")

# 指定 max_cat 為無序的類別變數
test_data$max_cat <- factor(test_data$max_cat, levels = c("0","1","A","B","C","D","E","F","G","H","I","J","K","L","O","V", "_"))
